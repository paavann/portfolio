    ...
    return (
        <div className="w-full bg-[rgb(0_21_36)] p-5 sm:p-7 lg:p-10 flex flex-col gap-7 sm:gap-10">
            <div className="p-3 sm:p-5 h-fit w-full gap-4 sm:gap-7 flex flex-col">
                <h1 className="text-3xl sm:text-4xl lg:text-5xl text-white font-black">blogs.</h1>
                <h2 className="text-lg sm:text-2xl lg:text-3xl text-white font-semibold">
            ...
            {loading ? (
                ...
                <div ref={containerRef} className="w-full flex flex-wrap gap-5 sm:gap-7 lg:gap-10 mt-2 sm:mt-5 px-2 sm:px-5">
                    {blogs.map((blog) => (
                        <Link to={`/blogs/${blog.slug}`} key={blog.id} className="w-full md:w-[45%] lg:w-[30%]">
                            <TiltCard className="blog-card h-full w-full opacity-0">
